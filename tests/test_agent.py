...
from ahaocode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from ahaocode.client import LLMClient
from ahaocode.conversation import ConversationManager
from ahaocode.serialization import build_anthropic_messages
from ahaocode.tools import create_default_registry
from ahaocode.tools.base import (
    StreamEnd,
    StreamEvent,
    TextDelta,
    ToolCallComplete,
...
async def test_single_step_tool_call():
    ...
    client = MockLLMClient([
        ...
        [
            ...
            ToolCallComplete("t1", "ReadFile", {"file_path": "AHAOCODE.md"}),
    ...
    conv.add_user_message("Read AHAOCODE.md")
...
async def test_multi_step_autonomous():
    ...
    test_file = "/tmp/ahaocode_test_hello.txt"
    ...
    client = MockLLMClient([
        ...
        [
            ...
            ToolCallComplete("t1", "WriteFile", {"file_path": "/tmp/ahaocode_test_hello.txt", "content": "Hello World"}),
            StreamEnd("end_turn", input_tokens=10, output_tokens=20),
        ],
        # 第 2 轮：ReadFile 进行验证
        [
            TextDelta("Verifying content."),
            ToolCallComplete("t2", "ReadFile", {"file_path": "/tmp/ahaocode_test_hello.txt"}),
...
async def test_stop_max_iterations():
    ...
    for i in range(5):
        responses.append([
            TextDelta(f"Step {i}"),
            ToolCallComplete(f"t{i}", "ReadFile", {"file_path": "AHAOCODE.md"}),
...
async def test_stop_cancel():
    ...
    class SlowMockClient(LLMClient):
        ...
        async def stream(
            ...
            await asyncio.sleep(0.01)
            yield ToolCallComplete(f"t{self._call_count}", "ReadFile", {"file_path": "AHAOCODE.md"})
            await asyncio.sleep(0.01)
...
async def test_token_usage_accumulates():
    ...
    client = MockLLMClient([
        ...
        [
            TextDelta("Step 2"),
            ToolCallComplete("t2", "ReadFile", {"file_path": "AHAOCODE.md"}),
...
def test_partition_tool_calls():
    """分批逻辑会把可并发执行的调用归到同一组。"""
    from ahaocode.tools.base import ToolCallComplete
