...
需要环境变量 AHAOCODE_TEST_API_KEY、AHAOCODE_TEST_BASE_URL、AHAOCODE_TEST_MODEL。
...
def test_lock_first_acquire():
    from ahaocode.memory.consolidation import _read_last_consolidated_at, _try_acquire_lock
...
def test_lock_blocks_when_held():
    from ahaocode.memory.consolidation import _try_acquire_lock
...
def test_lock_rollback_deletes_on_zero():
    from ahaocode.memory.consolidation import _try_acquire_lock, _rollback_lock
...
def test_prompt_contains_all_phases():
    from ahaocode.memory.consolidation import _build_consolidation_prompt
...
@pytest.mark.skipif(
    not os.environ.get("AHAOCODE_TEST_API_KEY"),
    reason="AHAOCODE_TEST_API_KEY not set"
)
@pytest.mark.timeout(120)
def test_e2e_consolidation_merges_duplicates():
    api_key = os.environ["AHAOCODE_TEST_API_KEY"]
    base_url = os.environ.get("AHAOCODE_TEST_BASE_URL", "https://api.minimaxi.com/v1")
    model = os.environ.get("AHAOCODE_TEST_MODEL", "MiniMax-M3")

    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".ahaocode", "memory")
...
async def _run_consolidation(work_dir, api_key, base_url, model, mem_dir):
    from ahaocode.memory.consolidation import _build_consolidation_prompt
    from ahaocode.agent import Agent
    from ahaocode.conversation import ConversationManager
    from ahaocode.permissions.checker import PermissionChecker
    from ahaocode.tools import ToolRegistry
    from ahaocode.tools.bash import Bash
    from ahaocode.tools.edit_file import EditFile
    from ahaocode.tools.glob import Glob
    from ahaocode.tools.grep import Grep
    from ahaocode.tools.read_file import ReadFile
    from ahaocode.tools.write_file import WriteFile

    from ahaocode.config import ProviderConfig
    from ahaocode.client import OpenAICompatClient
    ...
    from ahaocode.permissions.sandbox import PathSandbox
    from ahaocode.permissions.rules import RuleEngine
    from ahaocode.permissions.dangerous import DangerousCommandDetector
    from ahaocode.permissions.checker import PermissionMode
