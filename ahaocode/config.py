...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".ahaocode" / "config.yaml",
        cwd / ".ahaocode" / "config.yaml",
        cwd / ".ahaocode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .ahaocode/config.yaml "
            "in project or ~/.ahaocode/config.yaml"
        )
    return merged
