...
def _session_path(ahaocode_dir: Path) -> Path:
    return ahaocode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(ahaocode_dir)
    if session is None:
...
def load_worktree_session(ahaocode_dir: Path) -> WorktreeSession | None:
    path = _session_path(ahaocode_dir)
    if not path.exists():
        return None
    try:
