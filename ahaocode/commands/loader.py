...
"""从 .ahaocode/commands/ 目录加载自定义 Markdown 命令（对齐 Go 版 loader.go）。
...
from ahaocode.commands.registry import Command, CommandContext, CommandType
...
def load_user_commands(work_dir: str) -> list[Command]:
    ...
    搜索路径（后者覆盖前者）：
      1. ~/.ahaocode/commands/
      2. <work_dir>/.ahaocode/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".ahaocode" / "commands"))
    dirs.append(str(Path(work_dir) / ".ahaocode" / "commands"))
