...
from ahaocode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from ahaocode.conversation import ThinkingBlock as ConvThinkingBlock
from ahaocode.memory.auto_memory import MemoryManager
from ahaocode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from ahaocode.hooks import HookContext, HookEngine, ToolRejectedError
from ahaocode.hooks.engine import HookNotification
...
class Agent:
    def __init__(
        ...
        if memory_manager is not None:
            from ahaocode.memory.consolidation import MemoryConsolidator
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".ahaocode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".ahaocode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from ahaocode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from ahaocode.context.manager import (
