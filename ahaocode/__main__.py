...
from ahaocode.config import ConfigError, load_config
from ahaocode.hooks import HookConfigError, HookEngine, load_hooks
from ahaocode.permissions import PermissionMode


def main() -> None:
    # 先确保 .ahaocode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".ahaocode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".ahaocode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="ahaocode", description="Ahaocode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from ahaocode.remote import RemoteServer
    ...
    from ahaocode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from ahaocode.client import create_client, resolve_context_window
    from ahaocode.conversation import ConversationManager
    from ahaocode.memory.instructions import load_instructions
    ...
    from ahaocode.tools import create_default_registry
