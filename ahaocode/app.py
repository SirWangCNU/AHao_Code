...
from ahaocode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from ahaocode.commands.completion import CompletionPopup
from ahaocode.commands.handlers import register_all_commands
from ahaocode.config import MCPServerConfig, ProviderConfig
from ahaocode.hooks import HookContext, HookEngine, load_hooks
from ahaocode.conversation import ConversationManager, Message
from ahaocode.mcp import ConnectResult, MCPManager
from ahaocode.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from ahaocode.permissions import (
...
from ahaocode.agents.task_manager import TaskManager
from ahaocode.agents.trace import TraceManager
from ahaocode.agents.notification import inject_task_notifications
from ahaocode.commands.handlers.tasks import create_tasks_command
from ahaocode.skills.executor import SkillExecutor
from ahaocode.skills.loader import SkillLoader
from ahaocode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from ahaocode.cache import FileCache
from ahaocode.tools import ToolRegistry, create_default_registry
from ahaocode.tools.agent_tool import AgentTool
from ahaocode.tools.ask_user import AskUserEvent, AskUserTool
from ahaocode.tools.impl.tool_search import ToolSearchTool
from ahaocode.tools.install_skill import InstallSkillTool
from ahaocode.tools.load_skill import LoadSkill
from ahaocode.worktree.cleanup import start_stale_cleanup_task
from ahaocode.worktree.manager import WorktreeManager
from ahaocode.commands.handlers.worktree import create_worktree_command
from ahaocode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".ahaocode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".ahaocode" / "history"
...
class AhaocodeApp(App):
    ...
    def __init__(
        ...
        from ahaocode.config import SandboxAppConfig
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        ...
        model_line = model or "No provider selected"
        path_line = work_dir or os.getcwd()

        lines = [
            ("╭─ Ahaocode v0.2.0 " + "─" * (total_width - 18) + "╮", "bold color(210)"),
            (
                "│"
                + "Welcome back!".center(left_width)
                + "│ "
                + "Tips for getting started".ljust(right_width)
                + "│",
                "bold color(255)",
            ),
            (
                ...
                + "".center(left_width)
                + "│ "
                + "Run /init to create an AHAOCODE.md file with instructions".ljust(right_width)
                + "│",
                "color(255)",
            ),
            (
            ...
            (
                "│"
                + "    \\___/    ".center(left_width)
                + "│ "
                + "What's new".ljust(right_width)
                + "│",
                "bold color(210)",
            ),
            (
                "│"
                + model_line[:left_width].center(left_width)
                + "│ "
                + "Added /cd command to move sessions between worktrees".ljust(right_width)
                + "│",
                "color(250)",
            ),
            (
                "│"
                + path_line[:left_width].center(left_width)
                + "│ "
                + "Added safer bypass-permissions checks and clearer prompts".ljust(right_width)
                + "│",
                "color(250)",
            ),
            ("╰" + "─" * total_width + "╯", "bold color(210)"),
        ]
    ...
    def on_mount(self) -> None:
        self.register_theme(_AHAOCODE_THEME)
        self.theme = "ahaocode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".ahaocode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".ahaocode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".ahaocode" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from ahaocode.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.ahaocode/config.yaml",
                        f"{work_dir}/.ahaocode/permissions.local.yaml",
                    ],
        ...
        from ahaocode.filehistory import FileHistory
        ...
        from ahaocode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from ahaocode.config import WorktreeConfig
        ...
        from ahaocode.tools.enter_worktree import EnterWorktreeTool
        from ahaocode.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from ahaocode.teams.manager import TeamManager
        from ahaocode.tools.team_create import TeamCreateTool
        from ahaocode.tools.team_delete import TeamDeleteTool
        ...
        from ahaocode.commands.handlers.trace import create_trace_command
        ...
        from ahaocode.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from ahaocode.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from ahaocode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from ahaocode.plan_dialog import InlinePlanWidget, PlanChoice
        from ahaocode.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from ahaocode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from ahaocode.permission_dialog import InlinePermissionWidget
